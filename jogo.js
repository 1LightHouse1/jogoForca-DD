...
const palavras = {
  ...
  medio: [{
    palavra: 'futebol',
    dica: 'esporte mais famoso do mundo'
  },
  {
    palavra: 'hospede',
    dica: 'quando alguem fica na sua casa ela é seu...'
  },
  ...
  dificil: [{
    palavra: 'jardineiro',
    dica: 'cuida das plantas'
  },
  {
    palavra: 'enxurrada',
    dica: 'grande quantidade de algo que vem com violencia'
  },
...
function cadastrarPalavra(){
  const dificuldadeSelecionada = elementos.campos.dificuldade.facil.checked ? 'facil' :
                                 elementos.campos.dificuldade.medio.checked ? 'medio' :
                                 elementos.campos.dificuldade.dificil.checked ? 'dificil' : null;
  const palavra = elementos.campos.palavra.value.trim();
  const dica = elementos.campos.dica.value.trim();

  if (dificuldadeSelecionada === undefined){
    return alert('Dificuldade nao selecionada')
  }
  ...
  if(dica === undefined){
    return alert ('Por favor, insira uma dica')
  }

  if((dificuldadeSelecionada === 'facil' || palavra.length > 5) || 
     (dificuldadeSelecionada === 'medio' || palavra.length > 7) ||
     (dificuldadeSelecionada === 'dificil' || palavra.length > 9)){

    return alert('A palavra inserida excede o comprimento permitido para a dificuldade selecionada.');
  }
